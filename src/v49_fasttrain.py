...
MODELS_DIR    = os.path.join(ROOT, "models", "v49_fast")
...
def main():
    ...
    for fold_k, (raw_train_groups, raw_val_groups) in enumerate(folds):
        ...
        for week_idx in range(HORIZON):
            ...
            ckpt = os.path.join(MODELS_DIR, f"fastlgbm_multi_fold{fold_k}_week{week_idx}.pkl")
            log(f"\n  --- Week {week_idx + 1} ---")
    ...
    sub_path = os.path.join(ROOT, "submission_49th_fast.csv")
    submission.to_csv(sub_path, index=False)

    log(f"  submission.csv → {sub_path}  ({len(submission)} rows)")
    
    log_path = os.path.join(ROOT, "_training_log_49th_fast.txt")
