...
MODELS_DIR    = os.path.join(ROOT, "v51_models")
...
def main():
    ...
    log(f"Feature space: {len(FEATURE_COLS)} features x {WINDOW_SIZE} weeks + "
    ...
    log(f"  flat input dim: {N_FLAT_FEATURES}  "
        f"({WINDOW_SIZE}x{len(FEATURE_COLS)}+{len(FEATURE_COLS)})")
    ...
    for fold_k, (raw_train_groups, raw_val_groups) in enumerate(folds):
        ...
        for week_idx in range(HORIZON):
            ...
            if model_b is None:
                ...
                model_b = LGBMClassifier(               #v45_30k
                    objective        = "binary",
                    max_depth        = 6,
                    num_leaves       = 45,
                    colsample_bytree = 0.6,
