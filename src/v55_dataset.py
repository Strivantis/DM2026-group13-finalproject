...
# [V55] V54 物理特徵 + 恢復 Target Encoding (per-fold leak-free)
FEATURE_COLS = [
    # --- 基礎氣象 ---
    ...
    # --- [V55] Target Encoding (歷史先驗，免疫 covariate shift) ---
    "region_mean_score",
    "region_zero_prob",
]
...
def refine_features(df: pd.DataFrame, is_train: bool = True) -> pd.DataFrame:
    ...
    # TE columns (region_mean_score, region_zero_prob) are injected later by the
    # training loop per fold; exclude them here to avoid spurious NaN drops.
    present_feats = [c for c in FEATURE_COLS
                     if c in df.columns and not c.startswith("region_")]
...
# ---------------------------------------------------------------------------
# TE Leakage-Prevention Helper (used by v55_train.py)
# ---------------------------------------------------------------------------
def extract_training_targets_for_te(train_groups: list) -> pd.DataFrame:
    """
    Extracts scores ONLY from the training timeframe (up to i_max + window + horizon)
    to prevent future leakage when computing region_mean_score / region_zero_prob.
    """
    scores, rids = [], []
