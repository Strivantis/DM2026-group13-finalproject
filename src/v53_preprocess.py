"""
v53_preprocess.py – Full Preprocessing Pipeline (v53)
=========================================================
Pipeline steps (daily → weekly → processed CSVs):
  ...
  7.  Drought index & v53 Strategic Features:
      * [NEW] water_shortage: Pure positive indicator max(0, pet - prec)
      * [NEW] vpd_proxy: tmp * (100 - humidity) / 100
...
_PROC = os.path.join(_DATA, "v53_processed") # [v53 修改] 匯出到專屬資料夾
...
# [v53 修改] 移除容易混淆的 deficit，改用 water_shortage
DROUGHT_FEAT_COLS = [
    "pet",
    "water_shortage",
...
# ---------------------------------------------------------------------------
# Drought Index & v53 Strategic Features
# ---------------------------------------------------------------------------
def add_drought_index(df: pd.DataFrame, is_train: bool = True) -> pd.DataFrame:
    ...
    # ---------------------------------------------------------------------
    # [v53 修正] 廢棄負數 deficit，改用純粹的正向缺水指標 water_shortage
    # ---------------------------------------------------------------------
    ...
    # 2. 蒸發動能 (VPD Proxy) - 高溫低濕複合特徵
    df["vpd_proxy"] = (df["tmp"] * (100.0 - df["humidity"]) / 100.0).astype(np.float32)

    # 3. 熱衝擊指數 (Heat Shock)
    if "tmp_week_max" in df.columns:
        df["heat_shock"] = (df["tmp_week_max"] - df["tmp"]).astype(np.float32)
        
    # 4. 區域氣候異常度 (Temperature Anomaly)
    if is_train:
    ...
    if not is_train:
        v53_new_cols = ["aridity_index", "vpd_proxy", "heat_shock", "tmp_anomaly"]
        for col in DROUGHT_FEAT_COLS + v53_new_cols:
            if col in df.columns:
                df[col] = df.groupby("region_id")[col].transform(
...
def main():
    ...
    print("=" * 75)
    print("Preprocessing Pipeline v53 (Strategic Features Added)")
    ...
    # ---- 7. Drought Index & v53 Features ------------------------------------
    print("\nComputing drought index & v53 features (water_shortage, vpd_proxy...) ...")
